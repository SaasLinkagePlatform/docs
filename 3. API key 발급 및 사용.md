...
- 연계서비스ID, API키 확인 방법: `로그인 -> 우측 상단의 운영 메뉴 -> API 관리 → API 인증키`에서 확인
- 암호화 키 확인 방법: `로그인 -> 우측 상단의 운영 메뉴 -> API 관리 → 암호화 키`에서 확인

---

### (2) 복호화키 생성
- 하단 복호화 방법 java 코드의 String decryptKey = decryptApiKey(API Key, 암호화키); 확인
- **API Key**에 `(1) 연계서비스ID / API 인증키 / 암호화 키 확인` 에서 확인한 `API 인증키 값` 붙여넣기
- **암호화키**에 `(1) 연계서비스ID / API 인증키 / 암호화 키 확인` 에서 확인한 `암호화키 값` 붙여넣기
- 데모 소스 코드를 실행시킨 후, 콘솔에 출력되는 값 `복호화키 값` 을 확인

> 인증 연계 API (모바일공무원인증, GPKI, EPKI 연계 API) 에서는 복호화 과정 없이 발급받은 API키를 client secret에 그대로 사용
---

### (3) API 호출
- 연계 API에 접근하기 위해서는 API Key 및 암호화 Key를 복호화하는 과정이 필요합니다.
- 포털을 통해 받은 정식 API Key 및 암호화 Key를 입력 받아, 플레인 텍스트 형태의 복호화된 API 키를 API 호출에 사용합니다.
- Postman, hoppscotch 등 자율적으로 오픈소스 API 테스트 사이트에서 사용 가능합니다.
- *각 연계서비스별로 서로 다른 연계서비스ID / API 인증키 / 암호화 키가 발급되므로 각 서비스마다 서로 다른 연계서비스ID 확인 및 복호화키 발급 과정 필요합니다.
...
- 개발 서버(gpki-stg)로 호출하여 GPKI 로그인 페이지가 호출되어야 합니다.
### 호출 예시5) EPKI
![img_1.png](image/API%20Key%20발급관련/10.API%20Key%20발급.png)
- 개발 서버(epki-stg)로 호출하여 EPKI 로그인 페이지가 호출되어야 합니다.

### ** 전자정부 연계 API 개발 가이드는 다음 링크를 참고해주세요
- https://github.com/privateSaasOperationSupportCenter/develop/tree/main/linkService/

---
### (4) 테스트키 인증 확인 및 실제키 신청
- 지금까지의 과정을 통하여 테스트키 인증이 완료되었으면 **실제키**로 전환이 필요합니다.
- 테스트키로는 통신에 대한 성공/실패 여부만 return 하기 때문에 실제 데이터는 실제키 전환 이후 확인 가능합니다.
- 실제키로 전환을 위하여 통합관리포털의 `신청관리 -> API 실제키 신청` 메뉴에서 ‘테스트키 통신 성공 증적’을 첨부하여 실제키 신청글을 작성해야합니다. (각 연계 서비스 별로 서로 다른 증적 첨부)
  - 실제키 신청 시, 증적 자료는 API 호출 이후 console 결과를 통한 통신 증적 (request 주소, 시간) 등을 확인하여 첨부합니다.
  - 운영지원센터 담당자의 실제키 발급 신청 승인 단계를 거친 후, 통합관리포털에서 새로 발급받은 연계서비스ID 및 키를 통해 (1),(2),(3)을 반복하여 실제 데이터 확인이 가능합니다.
...
- 이용기관 담당자의 허용데이터 목록 체크 여부에 따라 민간 SaaS에 제공되는 LDAP 허용데이터 범위 설정할 수 있습니다.
- LDAP 단일 연계시에는 기본적으로 `전체 조직명, 이름, 직급, 직위`를 제공받을 수 있습니다.
  - CN은 인증서비스(모바일공무원증, GPKI, EPKI)를 신청한 경우에 한하여 연계 여부 선택 가능합니다.
